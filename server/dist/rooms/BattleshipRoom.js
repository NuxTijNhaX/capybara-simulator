...
exports.BattleshipRoom = void 0;
const core_1 = require("@colyseus/core");
const BattleshipState_1 = require("../schemas/BattleshipState");
const BattleshipPlayer_1 = require("../schemas/BattleshipPlayer");
const SHIP_TYPES = [
    { name: "Carrier", size: 5 },
...
class BattleshipRoom extends core_1.Room {
    constructor() {
        super(...arguments);
        this.maxClients = 2;
        this.playerOrder = [];
    ...
    onCreate(options) {
        this.setState(new BattleshipState_1.BattleshipState());
        this.onMessage("placeShips", (client, data) => {
    ...
    onJoin(client, options) {
        ...
        const player = new BattleshipPlayer_1.BattleshipPlayer();
    ...
    handlePlaceShips(sessionId, ships) {
        const player = this.state.players.get(sessionId);
        if (!player || this.state.phase !== "placement")
            return;
        // Validate ship count
        if (ships.length !== 5) {
            const client = this.clients.find(c => c.sessionId === sessionId);
            if (client)
                client.send("error", { message: "Must place exactly 5 ships" });
            return;
        }
        // Reset board
        player.board = new Array(BOARD_SIZE * BOARD_SIZE).fill(0);
        ...
        for (const shipData of ships) {
            const ship = new BattleshipPlayer_1.Ship();
            ...
            if (!this.canPlaceShip(player.board, ship)) {
                const client = this.clients.find(c => c.sessionId === sessionId);
                if (client)
                    client.send("error", { message: `Cannot place ${ship.name}` });
                return;
            }
    ...
    handleReady(sessionId) {
        const player = this.state.players.get(sessionId);
        if (!player || !player.placed || player.ready)
            return;
        player.ready = true;
    ...
    handleFire(sessionId, x, y) {
        ...
        if (opponent.board[idx] === 2 || opponent.board[idx] === 3) {
            const client = this.clients.find(c => c.sessionId === sessionId);
            if (client)
                client.send("error", { message: "Already fired here" });
            return;
        }
        // Process shot
        let result;
    ...
    checkShipSunk(player, hitX, hitY) {
...
exports.BattleshipRoom = BattleshipRoom;
//# sourceMappingURL=BattleshipRoom.js.map