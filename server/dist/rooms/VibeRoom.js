...
exports.VibeRoom = void 0;
const core_1 = require("@colyseus/core");
const VibeState_1 = require("../schemas/VibeState");
...
class VibeRoom extends core_1.Room {
    constructor() {
        super(...arguments);
        this.maxClients = 50;
        // AFK timeout: 5 minutes
        this.AFK_TIMEOUT = 5 * 60 * 1000;
        // Update rate: 20fps (50ms)
        this.TICK_RATE = 50;
        // Active battleship challenges
        this.challenges = new Map();
        // Active caro challenges
        this.caroChallenges = new Map();
        // Active caro games
        this.caroGames = new Map();
        this.caroPlayerGames = new Map();
    }
    onCreate(options) {
        this.setState(new VibeState_1.VibeState());
        ...
        this.onMessage("battleshipChallenge", (client, targetSessionId) => {
            ...
            const existingChallenge = Array.from(this.challenges.values()).find(c => c.challengerId === client.sessionId || c.targetId === client.sessionId ||
        ...
        this.onMessage("battleshipAccept", async (client, challengeId) => {
            ...
            try {
                ...
                const battleRoom = await core_1.matchMaker.createRoom('battleship_room', {});
        ...
        this.onMessage("battleshipDecline", (client, challengeId) => {
            const challenge = this.challenges.get(challengeId);
            if (!challenge) {
                client.send("challengeError", { message: "Challenge not found" });
                return;
        ...
        this.onMessage("battleshipCancel", (client, challengeId) => {
            const challenge = this.challenges.get(challengeId);
            if (!challenge)
                return;
            if (client.sessionId !== challenge.challengerId)
                return;
        ...
        this.onMessage("caroChallenge", (client, targetSessionId) => {
            const challenger = this.state.players.get(client.sessionId);
            const target = this.state.players.get(targetSessionId);
            if (!challenger || !target) {
            ...
            // Check if either player is already in a game or has pending challenge
            const existingChallenge = Array.from(this.caroChallenges.values()).find(c => c.challengerId === client.sessionId || c.targetId === client.sessionId ||
                c.challengerId === targetSessionId || c.targetId === targetSessionId);
            if (existingChallenge) {
        ...
        this.onMessage("caroAccept", (client, challengeId) => {
        ...
        this.onMessage("caroDecline", (client, challengeId) => {
            const challenge = this.caroChallenges.get(challengeId);
            if (!challenge) {
                client.send("caroError", { message: "Challenge not found" });
                return;
    ...
    checkCaroWin(board, row, col, pieceValue) {
        const directions = [
            [[0, 1], [0, -1]], // horizontal
            [[1, 0], [-1, 0]], // vertical
            [[1, 1], [-1, -1]], // diagonal \
...
exports.VibeRoom = VibeRoom;
//# sourceMappingURL=VibeRoom.js.map